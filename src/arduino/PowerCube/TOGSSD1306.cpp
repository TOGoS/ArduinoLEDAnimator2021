...
void TOGSSD1306::setFont(const uint8_t* font)
{
  m_font = font;
  m_font_width = pgm_read_byte(&m_font[0]);
}

void TOGSSD1306::sendCommand(unsigned char command)
{
...
void TOGSSD1306::setBrightness(unsigned char Brightness)
{
   sendCommand(SSD1306_Set_Brightness_Cmd);
   sendCommand(Brightness);
}

void TOGSSD1306::setHorizontalMode()
{
...
bool TOGSSD1306::putChar(unsigned char ch)
{
    if (!m_font) return 0;
...
void TOGSSD1306::putString(const char *string)
{
    unsigned char i=0;
    while(string[i])
    {
...
void TOGSSD1306::putString(String string)
{
...
unsigned char TOGSSD1306::putNumber(long long_num)
{
...
unsigned char TOGSSD1306::putFloat(float floatNumber,unsigned char decimal)
{
  unsigned int temp=0;
  float decy=0.0;
  float rounding = 0.5;
...
unsigned char TOGSSD1306::putFloat(float floatNumber)
{
...
void TOGSSD1306::drawBitmap(unsigned char *bitmaparray,int bytes)
{
...
void TOGSSD1306::setHorizontalScrollProperties(bool direction,unsigned char startPage, unsigned char endPage, unsigned char scrollSpeed)
{
...
void TOGSSD1306::activateScroll()
{
    sendCommand(SSD1306_Activate_Scroll_Cmd);
}

void TOGSSD1306::deactivateScroll()
{
    sendCommand(SSD1306_Dectivate_Scroll_Cmd);
}

void TOGSSD1306::setNormalDisplay()
{
    sendCommand(SSD1306_Normal_Display_Cmd);
}

void TOGSSD1306::setInverseDisplay()
{
    sendCommand(SSD1306_Inverse_Display_Cmd);
}


TOGSSD1306 oled;  // Pre-instantiate object
